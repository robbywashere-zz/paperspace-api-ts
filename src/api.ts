...
export type MachineStateType = "ready" | "serviceready" | "off";
...
export class PaperspaceApi {
   ...
   * @param {string} password - Password for the specified email address  * @name PaperspaceApi#Authenticate
   */
  ...
  async Authenticate({
    ...
    const {
      body: { key }
    } = await this.LoginUser({ apiToken, email, password });
    this.SetToken(key);
  }
  /**
