...
export class PaperspaceApi {
  ...
  constructor(domain: string | null, private logger?: Logger) {
    if (domain) {
      this.domain = domain;
    }
  }
  ...
  /**
   * Login and set api token
   * @method
   * @name PaperspaceApi#Authenticate
   */

  async Authenticate({ apiToken, email, password }:{
    apiToken?: string;
    email: string;
    password: string;
  }) {
    const { body: { key }} = await this.LoginUser({ apiToken, email, password });
    this.SetToken(key);
  }
  /**
   * Set requests x-api-key headers for subsequent requests / @see PaperspaceApi#LoginUser 
   * @method
   * @name PaperspaceApi#SetToken
   * @param {string} apiKey - x-api-key headers
   */
  ...
  static async WaitFor<T>(
    ...
    await (async function wait(count = 0): Promise<void> {
      if (to.retry && to.retry >= count)
        throw new Error(`::: PaperspaceApi#WaitFor - timeout ${to.delay * count}ms`);
      let res = await fn();
  ...
  GetJob(parameters: {
    ...
    return new Promise((resolve, reject) => {
      ...
      const queryParams: Array<string> = ["jobId"];
      queryParams.forEach(qp => {
        if (qp in parameters) queryParameters[qp] = (parameters as any)[qp];
      });
      if (parameters.$queryParameters) {
  ...
  /**
   * Get a machine for the given id
   * @method
   * @name PaperspaceApi#GetMachine
   * @param {string} machineId - Id of the machine to wait for
   */
  GetMachine(parameters: {
    machineId: string;
    $queryParameters?: any;
    $domain?: string;
    ...
    return new Promise((resolve, reject) => {
      ...
      const queryParams: Array<string> = ["machineId"];
      queryParams.forEach(qp => {
        if (qp in parameters) queryParameters[qp] = (parameters as any)[qp];
      });
      if (parameters.$queryParameters) {
